<div class="memos index">
<h2><?php __('Memos');?></h2>
<p>
<?php
echo $paginator->counter(array(
'format' => __('Page %page% of %pages%, showing %current% records out of %count% total, starting on record %start%, ending on %end%', true)
));
?></p>
<table cellpadding="0" cellspacing="0">
<tr>
	<th><?php echo $paginator->sort('id');?></th>
	<th><?php echo $paginator->sort('user_id');?></th>
	<th><?php echo $paginator->sort('sender_id');?></th>
	<th><?php echo $paginator->sort('created');?></th>
	<th><?php echo $paginator->sort('lastupdated');?></th>
	<th><?php echo $paginator->sort('flag');?></th>
	<th><?php echo $paginator->sort('sender');?></th>
	<th><?php echo $paginator->sort('content');?></th>
	<th class="actions"><?php __('Actions');?></th>
</tr>
<?php
$i = 0;
foreach ($memos as $memo):
	$class = null;
	if ($i++ % 2 == 0) {
		$class = ' class="altrow"';
	}
?>
	<tr<?php echo $class;?>>
		<td>
			<?php echo $memo['Memo']['id']; ?>
		</td>
		<td>
			<?php echo $html->link($memo['User']['id'], array('controller'=> 'users', 'action'=>'view', $memo['User']['id'])); ?>
		</td>
		<td>
			<?php echo $memo['Memo']['sender_id']; ?>
		</td>
		<td>
			<?php echo $memo['Memo']['created']; ?>
		</td>
		<td>
			<?php echo $memo['Memo']['lastupdated']; ?>
		</td>
		<td>
			<?php echo $memo['Memo']['flag']; ?>
		</td>
		<td>
			<?php echo $memo['Memo']['sender']; ?>
		</td>
		<td>
			<?php echo $memo['Memo']['content']; ?>
		</td>
		<td class="actions">
			<?php echo $html->link(__('View', true), array('action'=>'view', $memo['Memo']['id'])); ?>
			<?php echo $html->link(__('Edit', true), array('action'=>'edit', $memo['Memo']['id'])); ?>
			<?php echo $html->link(__('Delete', true), array('action'=>'delete', $memo['Memo']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $memo['Memo']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
</table>
</div>
<div class="paging">
	<?php echo $paginator->prev('<< '.__('previous', true), array(), null, array('class'=>'disabled'));?>
 | 	<?php echo $paginator->numbers();?>
	<?php echo $paginator->next(__('next', true).' >>', array(), null, array('class'=>'disabled'));?>
</div>
<div class="actions">
	<ul>
		<li><?php echo $html->link(__('New Memo', true), array('action'=>'add')); ?></li>
		<li><?php echo $html->link(__('List Users', true), array('controller'=> 'users', 'action'=>'index')); ?> </li>
		<li><?php echo $html->link(__('New User', true), array('controller'=> 'users', 'action'=>'add')); ?> </li>
	</ul>
</div>
