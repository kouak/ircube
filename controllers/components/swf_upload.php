...
class SwfUploadComponent extends Object {
	...
	var $filename;
	var $mimetype = null;
	var $bypassMimeValidationOnLookupFailure = false;
	 ...
	 */
	function upload($allowedMimeType=null) {
		$ok = false;
		if ($this->validate($allowedMimeType)) {
	 ...
	 */
	function validate($allowedMimeType = null) {
		...
		if (!$post_ok){
			$this->setError(2000, __('Une erreur s\'est produite.', true), 'Expected file upload field to be named "Filedata."');
			return false;
		}
		if ($upload_error){
			$this->setError(2500, __('Une erreur s\'est produite', true), $this->getUploadErrorMessage($upload_error));
			return false;
		}
		...
		if($got_data) {
			...
			if(function_exists("finfo_open")) {
				...
				$this->mimetype = finfo_file($finfo, $this->params['form']['Filedata']['tmp_name']);
			} elseif (function_exists("mime_content_type")) {
				$this->mimetype = mime_content_type($this->params['form']['Filedata']['tmp_name']);
			} elseif (function_exists("getimagesize") && in_array('img', $allowedMimeType)) {
				$size = getimagesize($this->params['form']['Filedata']['tmp_name']);
				if(!empty($size)) {
					$this->mimetype = $size['mime'];
				}
			}
			if($this->mimetype === null) {
				$this->log('Mimetype couldn\'t be found, falling back to form submitted value');
				$this->mimetype = $this->params['form']['Filedata']['type'];
				if($this->bypassMimeValidationOnLookupFailure || 1) {
					$this->log('Bypassing mime type validation ...');
					return true;
				}
			}
			if (is_array($allowedMimeType)) {
				$err = true;
				foreach ($allowedMimeType as $mimeType) {
					if(in_array($this->mimetype, $this->allowedMime[$mimeType])) {
						$err = false;
					}
				}
				if ($err === true) {
					$this->setError(2000, __('Ce type de fichier est interdit.', true));
					return false;
				}
			}
			return true;
