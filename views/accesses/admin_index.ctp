<div class="accesses index">
<h2><?php __('Accesses');?></h2>
<p>
<?php
echo $paginator->counter(array(
'format' => __('Page %page% of %pages%, showing %current% records out of %count% total, starting on record %start%, ending on %end%', true)
));
?></p>
<table cellpadding="0" cellspacing="0">
<tr>
	<th><?php echo $paginator->sort('id');?></th>
	<th><?php echo $paginator->sort('flag');?></th>
	<th><?php echo $paginator->sort('created');?></th>
	<th><?php echo $paginator->sort('modified');?></th>
	<th><?php echo $paginator->sort('lastseen');?></th>
	<th><?php echo $paginator->sort('channel_id');?></th>
	<th><?php echo $paginator->sort('user_id');?></th>
	<th><?php echo $paginator->sort('channel_name');?></th>
	<th><?php echo $paginator->sort('user_name');?></th>
	<th><?php echo $paginator->sort('level');?></th>
	<th><?php echo $paginator->sort('info');?></th>
	<th class="actions"><?php __('Actions');?></th>
</tr>
<?php
$i = 0;
foreach ($accesses as $access):
	$class = null;
	if ($i++ % 2 == 0) {
		$class = ' class="altrow"';
	}
?>
	<tr<?php echo $class;?>>
		<td>
			<?php echo $access['Access']['id']; ?>
		</td>
		<td>
			<?php echo $access['Access']['flag']; ?>
		</td>
		<td>
			<?php echo $access['Access']['created']; ?>
		</td>
		<td>
			<?php echo $access['Access']['modified']; ?>
		</td>
		<td>
			<?php echo $access['Access']['lastseen']; ?>
		</td>
		<td>
			<?php echo $html->link($access['Channel']['id'], array('controller'=> 'channels', 'action'=>'view', $access['Channel']['id'])); ?>
		</td>
		<td>
			<?php echo $html->link($access['User']['id'], array('controller'=> 'users', 'action'=>'view', $access['User']['id'])); ?>
		</td>
		<td>
			<?php echo $access['Access']['channel_name']; ?>
		</td>
		<td>
			<?php echo $access['Access']['user_name']; ?>
		</td>
		<td>
			<?php echo $access['Access']['level']; ?>
		</td>
		<td>
			<?php echo $access['Access']['info']; ?>
		</td>
		<td class="actions">
			<?php echo $html->link(__('View', true), array('action'=>'view', $access['Access']['id'])); ?>
			<?php echo $html->link(__('Edit', true), array('action'=>'edit', $access['Access']['id'])); ?>
			<?php echo $html->link(__('Delete', true), array('action'=>'delete', $access['Access']['id']), null, sprintf(__('Are you sure you want to delete # %s?', true), $access['Access']['id'])); ?>
		</td>
	</tr>
<?php endforeach; ?>
</table>
</div>
<div class="paging">
	<?php echo $paginator->prev('<< '.__('previous', true), array(), null, array('class'=>'disabled'));?>
 | 	<?php echo $paginator->numbers();?>
	<?php echo $paginator->next(__('next', true).' >>', array(), null, array('class'=>'disabled'));?>
</div>
<div class="actions">
	<ul>
		<li><?php echo $html->link(__('New Access', true), array('action'=>'add')); ?></li>
		<li><?php echo $html->link(__('List Channels', true), array('controller'=> 'channels', 'action'=>'index')); ?> </li>
		<li><?php echo $html->link(__('New Channel', true), array('controller'=> 'channels', 'action'=>'add')); ?> </li>
		<li><?php echo $html->link(__('List Users', true), array('controller'=> 'users', 'action'=>'index')); ?> </li>
		<li><?php echo $html->link(__('New User', true), array('controller'=> 'users', 'action'=>'add')); ?> </li>
	</ul>
</div>
